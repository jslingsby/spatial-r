...
Raster data are essentially data stored in a regular grid of pixels (or cells). The value of each pixel is a number representing a measured value (e.g. continuous data such as sea surface temperature) or a category (e.g. discrete data such as land cover class). All pixels have a value, even if the value is "No Data". Digital images like jpeg or png files are essentially rasters without spatial information. Rasters are particularly useful for representing continuous data. Discrete data are often better presented as vector polygons. For example:
