...
You will need to do your analyses in an RMarkdown (.Rmd) document and submit it in a public Git Repository. Make sure all your code chunks are visible. Name your .Rmd file as `README.Rmd` and add `output: rmarkdown::github_document` to the header of that file. Have a look at [this repository](https://github.com/jslingsby/test_BIO3019Prac/blob/main/README.Rmd) for an example. This will create a `README.md` file with all your code and figures that is visible when one opens the repository link online. 

Note that the repo does not have to be reproducible, but the code should be visible, you should state what you set out to do and what datasets you used (citing their sources and/or providing links to where you got them from). Do not upload large or sensitive data to the repository - GitHub won't handle large data. If you want/need to have the data in your local version of the Git repo on your laptop, then put it in a folder named `data` or similar and add a line to your `.gitignore` file with the name of the folder, e.g. `data/*` - see example [here](https://github.com/jslingsby/bogusrepo/blob/main/.gitignore). You can just download the file and add it to your Git repo if you didn't create a .gitignore when you created the repo.
