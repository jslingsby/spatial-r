...
```{r, echo = F}
unlink("data/cape_peninsula/veg/Vegetation_Indigenous_duplicate.shp") # Deletes the file
```
...
Why 5 plots and not one? This is because _sf_ wants to plot the properties of each attribute in the attribute table. Fortunately, there were only 5, but thre could have been hundreds! You can select the one you want with indexing like so:
...
Here's we'll apply the function `st_crop`. To use the function you need an object to crop, and an extent or bounding box to crop to. _sf_ is clever, and you can set the extent by giving it another object who's extent you'd like to match (check the bounding box given when we read in the data earlier). 
...
```{r}
veg <- st_crop(veg, ext) #Not that I'm overwriting the old data object
